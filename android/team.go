...
func shengli_screenshot() image.Image {
	out, err := exec.Command("adb","shell"," /system/bin/screencap", "-p").Output()
	if err != nil {
		println(err.Error())
	}
	pic, err := png.Decode(bytes.NewReader(out))
	rgbimage := pic.(*image.NRGBA)
	subimage := rgbimage.SubImage(image.Rect(787,53,1105,230)).(*image.NRGBA)
	if err != nil {
		log.Println("png 截图解码失败。")
	}
	return subimage
}
func team_screenshot() image.Image {
	out, err := exec.Command("adb","shell"," /system/bin/screencap", "-p").Output()
	if err != nil {
		println(err.Error())
	}
	pic, err := png.Decode(bytes.NewReader(out))
	rgbimage := pic.(*image.NRGBA)
	subimage := rgbimage.SubImage(image.Rect(1064,586,1222,620)).(*image.NRGBA)
	if err != nil {
		log.Println("png 截图解码失败。")
	}
	return subimage
...
func team_touch(){
	...
	cos1, _err := imgo.CosineSimilarity("jumpAndroid.png", "samsung_hun10_team.png")
	cos1_1w := cos1 * 10000
	if _err != nil {
		println(_err.Error())
	}
	if (cos1_1w > 9900.0) {
		...
		touchX, touchY := strconv.Itoa(jump.Random(1064, 1222)), strconv.Itoa(jump.Random(586, 620))
...
func shengli_touch(){
	pic := shengli_screenshot()
	jump.SavePNG("jumpAndroid.png",pic)
	cos2,_err:= imgo.CosineSimilarity("jumpAndroid.png","samsung_shengli.png")
	cos2_1w:=cos2*10000
	if _err!=nil{
		println(_err.Error())
	}
	...
	if(cos2_1w>9600.0) {
		//go jump.SavePNG("samsung_jiesuan.png",jiesuan_screenshot())
		touchX, touchY := strconv.Itoa(jump.Random(787, 1105)), strconv.Itoa(jump.Random(53, 230))
		touch_res, err2 := exec.Command("adb", "shell", "/system/bin/sh", "/system/bin/input", "swipe", touchX, touchY, touchX, touchY,
			strconv.Itoa(int(0.01*inputRatio))).Output()
		if err2 != nil {
			log.Fatal("模拟触摸失败，请检查开发者选项中的 USB 调试安全设置是否打开。")
